...
class VariationalNeuralNetwork:
    ...
    def __init__(self, x_values:List[List[int]], hidden_layers_neuron_amounts:List[int],
        ...
        # it is unnecesary to pass this value
        # just for security we get the encoder last layer / 2
        self.latent_dimension = int(hidden_layers_neuron_amounts[latent_layer_index-1] / 2)
        ...
        stats_folder = "stats/data/"
        if not os.path.exists(stats_folder):
            os.makedirs(stats_folder)
        self.filepath = os.path.join(stats_folder, filename) + ".csv"
        ...
        previous_layer_neurons = self.latent_dimension
    ...
    def feed_forward_through_network(self, input_values:List[int], layers:List[Layer], beta:float=1.0):
        a_j_vector = input_values
        for layer in layers:
            a_j_vector = layer.forward(a_j_vector, beta)
        return a_j_vector

    def make_reparametrization_trick(self, encoder_output:List[int]):
        mu = encoder_output[self.latent_dimension:]
        logvar = encoder_output[:self.latent_dimension]
        z = self.latent_space_layer.forward(mu, logvar)
        return z
    
    def predict(self, input_values:List[int], beta:float=1.0):
        encoder_output = self.feed_forward_through_network(input_values, self.encoder_layers, beta)
        z = self.make_reparametrization_trick(self, encoder_output)
        decoder_output = self.feed_forward_through_network(z, self.decoder_layers, beta)
    ...
    def backpropagate(self, input_values, target_values, learning_rate, epochs, optimizer, error_function, max_acceptable_error, is_adam_optimizer=False, activation_function="", activation_beta=1.0, alpha=0.0):
        ...
        f.write("epoch,minus_variational_lower_bound,reconstrucion_loss,kl_loss")
        for epoch in range(epochs):
            total_reconstruction_loss = 0
            total_kl_loss = 0

            for x in input_values:
                x_bar = self.predict(x, activation_beta)
                reconstruction_error = x - x_bar

                logvar = self.latent_space_layer.last_logvar
                mu = self.latent_space_layer.last_mu
                z = self.latent_space_layer.last_z
                epsilon = self.latent_space_layer.last_epsilon

                kl_loss = -0.5 * np.sum(1 + logvar - mu**2 - np.exp(logvar))

                total_reconstruction_loss += np.sum(reconstruction_error ** 2)
                total_kl_loss += kl_loss

                # --- Backward Pass ---
                # Decoder
                # not checked
                delta = reconstruction_error
            ...
            avg_recon = total_reconstruction_loss / len(input_values)
            ...
            minus_vlb = -total_reconstruction_loss + total_kl_loss
            f.write(f"{epoch+1},{minus_vlb},{total_reconstruction_loss},{total_kl_loss}")
